...
Point getReflectionPoint(Point l, Point n)
{
  return normalize(deductTwoPoint(scalarMultiplication(n, 2 * dotProduct(l, n)), l));
}
...
double calculateSphereLight(Point contact, Point center, Point v, double diffuse, double speculative, double shi, int chanel, double r)
{
  double finalColor = 0;
  Point n = normalize(scalarMultiplication(deductTwoPoint(contact, center), 1 / r));
  for (int i = 0; i < num_lights; i++)
  {
    Point l = normalize(deductTwoPoint(convertArrayToPoint(lights[i].position), contact));
    Point r = getReflectionPoint(l, n);
    finalColor += calculateLight(l, n, r, v, diffuse, speculative, shi, lights[i].color[chanel]);
  }
  return finalColor > 1 ? 0.99 : finalColor;
}
  ...
  for (int i = 0; i < num_spheres; i++)
  {
    ...
    else
    {
      ...
      if (tFinal > 0)
      {
        ...
        {
          ray.distance = tFinal;
          // double calculateSphereLight(Point contact, Point center, Point v, double diffuse, double speculative, double shi, int chanel)
          Point contactPoint = addTwoPoint(origin, scalarMultiplication(direction, tFinal));
          Point center = convertArrayToPoint(spheres[i].position);
          Point v = deductTwoPoint(ray.origin, contactPoint);
          ray.color[0] = calculateSphereLight(contactPoint, center, v, spheres[i].color_diffuse[0], spheres[i].color_specular[0], spheres[i].shininess, 0, r);
          ray.color[1] = calculateSphereLight(contactPoint, center, v, spheres[i].color_diffuse[1], spheres[i].color_specular[1], spheres[i].shininess, 1, r);
          ray.color[2] = calculateSphereLight(contactPoint, center, v, spheres[i].color_diffuse[2], spheres[i].color_specular[2], spheres[i].shininess, 2, r);
        }
        else if (tFinal < ray.distance)
        {
          ...
          Point contactPoint = addTwoPoint(origin, scalarMultiplication(direction, tFinal));
          Point center = convertArrayToPoint(spheres[i].position);
          Point v = deductTwoPoint(contactPoint, ray.origin);
          ray.color[0] = calculateSphereLight(contactPoint, center, v, spheres[i].color_diffuse[0], spheres[i].color_specular[0], spheres[i].shininess, 0, r);
          ray.color[1] = calculateSphereLight(contactPoint, center, v, spheres[i].color_diffuse[1], spheres[i].color_specular[1], spheres[i].shininess, 1, r);
          ray.color[2] = calculateSphereLight(contactPoint, center, v, spheres[i].color_diffuse[2], spheres[i].color_specular[2], spheres[i].shininess, 2, r);
        }
